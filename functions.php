...
if (!function_exists('bootstrapBasicEnqueueScripts')) {
	...
	function bootstrapBasicEnqueueScripts() 
	{
		...
		//<script async defer src="https://maps.googleapis.com/maps/api/js?key=AIzaSyAIoyAAIR8wooe6CMCoj-qXUSetwTiiyEU&callback=initMap" type="text/javascript"></script>
		//wp_enqueue_script( 'GMaps APIkey', 'https://maps.googleapis.com/maps/api/js?key=AIzaSyAIoyAAIR8wooe6CMCoj-qXUSetwTiiyEU&callback=initMap', array(), 'v3' );
		wp_enqueue_script( 'GMaps API', 'https://maps.googleapis.com/maps/api/js?v=3.exp&language=en&region=US', array(), 'v3' );
		wp_enqueue_script( 'jQuery', 'http://ajax.googleapis.com/ajax/libs/jquery/1.11.1/jquery.min.js', array(), '1.11.1' );
		wp_enqueue_script( 'Vimeo API', 'http://a.vimeocdn.com/js/froogaloop2.min.js', array(), null );
		wp_enqueue_script( 'Prefixfree', get_template_directory_uri() . '/js/prefixfree.min.js', array(), null );
		wp_enqueue_script( 'fullPagejs', get_template_directory_uri() . '/js/jquery.fullPage.min.js', array(), null );
		wp_enqueue_style( 'fullPagecss', get_template_directory_uri() . '/css/jquery.fullPage.css', array(), null );

		//wp_enqueue_style( 'gMaps', get_template_directory_uri() . '/css/wpgmza_style_pro.css', array(), null );
		//wp_enqueue_style( 'bootstrapcss', 'https://maxcdn.bootstrapcdn.com/bootstrap/3.3.6/css/bootstrap.min.css', array(), '3.3.6' );
		// Fi Afegits Roger
...
function iso_list_menu($format='none'){
    ...
    $languages = icl_get_languages('skip_missing=N&orderby=custom');
    ...
    if(!empty($languages)){
        $iso_lang_menu = NULL;
            foreach($languages as $l){
                ...
                $active_class   = ($l['active'] == 1 ? 'class="wpml_active_lang"' : NULL);          
                //$noactive_class   = (!$l['active'] == 1 ? 'class="wpml_noactive_lang"' : NULL);          
                $iso_lang_menu .= "<a href='".$l['url']."'".$active_class.">" . $lang_label . "</a>";
                $iso_lang_menu .= ' · ';  //used as spacer
                //$iso_lang_menu .= "<a href='".$l['url']."'".$noactive_class.">" . $lang_label . "</a>";                
            }
